...
/**
 * Container for the Wolf Tracker project that provides the user the ability to
 * interact with projects and logged tasks.
 * 
...
public class WolfTrackerGUI extends JFrame implements ActionListener {
	...
	/**
	 * Constructs a WolfTrackerGUI object that will contain a JMenuBar and a JPanel
	 * that will hold different possible views for the Wolf Tracker application.
	 */
	public WolfTrackerGUI() {
		...
		TitledBorder projectBorder = BorderFactory
				.createTitledBorder(BorderFactory.createEtchedBorder(EtchedBorder.LOWERED), "Project");
	...
	/**
	 * Makes the GUI Menu bar that contains options working with logged tasks,
	 * exporting summary stats, and quitting the application.
	 */
		...
		if (e.getSource() == itemNew) {
			if (project != null && project.isChanged()) {
				int select = JOptionPane.showConfirmDialog(null,
						"Current Project is unsaved. Would you like to save before creating a new Project?", "Warning",
			...
			try {
				if (project != null && project.isChanged()) {
					int select = JOptionPane.showConfirmDialog(null,
							"Current Project is unsaved. Would you like to save before creating a new Project?",
	...
	private class ProjectPanel extends JPanel {
		...
		public ProjectPanel() {
			...
			TitledBorder categoryBorder = BorderFactory
					.createTitledBorder(BorderFactory.createEtchedBorder(EtchedBorder.LOWERED), "Category");
			pnlCategory.setBorder(categoryBorder);
			pnlCategory.setToolTipText("Category");

			pnlStats = new StatsPanel();
			TitledBorder statsBorder = BorderFactory
					.createTitledBorder(BorderFactory.createEtchedBorder(EtchedBorder.LOWERED), "Summary Statistics");
			pnlStats.setBorder(statsBorder);
			pnlStats.setToolTipText("Summary Statistics");

			pnlTask = new TaskPanel();
			TitledBorder taskBorder = BorderFactory
					.createTitledBorder(BorderFactory.createEtchedBorder(EtchedBorder.LOWERED), "Task");
			pnlTask.setBorder(taskBorder);
			pnlTask.setToolTipText("Task");

			pnlLoggedTasks = new LoggedTasksPanel();
			TitledBorder loggedTasksBorder = BorderFactory
					.createTitledBorder(BorderFactory.createEtchedBorder(EtchedBorder.LOWERED), "Logged Tasks");
	...
	private class CategoryPanel extends JPanel implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == comboCategoryList) {
				...
				try {
					String categoryName = (String) JOptionPane.showInputDialog(this, "Category Name?",
							"Create new Category", JOptionPane.QUESTION_MESSAGE);
				...
				try {
					if (project.getCurrentLog() instanceof AllTasksLog) {
						...
						String categoryName = (String) JOptionPane.showInputDialog(this, "Update Category Name?",
								"Edit Category", JOptionPane.QUESTION_MESSAGE, null, null,
								project.getCurrentLog().getName());
				...
				try {
					if (project.getCurrentLog() instanceof AllTasksLog) {
						...
						int selection = JOptionPane.showOptionDialog(this,
								"Are you sure you want to delete " + project.getCurrentLog().getName() + "?",
	...
	private class TaskPanel extends JPanel implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == btnAddEdit) {
				try {
					if (idx == -1) {
						Task t = new Task(txtTaskTitle.getText(), Integer.parseInt(txtDuration.getText()),
								txtDescription.getText());
						project.addTask(t);
					} else {
						project.editTask(idx, txtTaskTitle.getText(), Integer.parseInt(txtDuration.getText()),
								txtDescription.getText());
					}
					setTask(-1);
				} catch (IllegalArgumentException iae) {
	...
	private class LoggedTasksPanel extends JPanel implements ActionListener {
		...
		/**
		 * TaskTableModel is the object underlying the JTable object that displays the
		 * list of logged Tasks to the user.
		 * 
